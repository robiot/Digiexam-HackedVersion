...
svgEditor.ready(function() {
	svgEditor.addExtension('dx_view_grid', function() { 'use strict';
		...
		function gridUpdate () {
			...
			$('#dx_view_grid').toggleClass('push_button_pressed tool_button nohover');
		}

		function setSnap(value) {
			svgEditor.curConfig.gridSnapping = value;
			svgCanvas.setConfig(svgEditor.curConfig);
		}

		return {
			svgicons: svgEditor.curConfig.extPath + 'ext-dx-grid.xml',
			buttons: [{
				id: 'dx_view_grid',
				...
				events: {
					click: function() {
						...
						if (!svgEditor.curConfig.showGrid) {
							internalSnapState = svgEditor.curConfig.gridSnapping
							setSnap(false);
						} else { setSnap(internalSnapState); }
			...
			callback: function () {
				...
				svgEditor.registerGlobalExtensionMethod("dx_view_grid", "setSnap", setSnap);

				var isTouch =  !!("ontouchstart" in window) || window.navigator.msMaxTouchPoints > 0;
				if (isTouch) {
					$('#dx_view_grid').addClass('nohover');
				}
			},
